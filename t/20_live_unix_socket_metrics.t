...
=== TEST 20: Live mode with Unix socket LAPI and metrics
...
{
        cs = require "crowdsec"
        local ok, err = cs.init("./t/conf_t/20_live_unix_socket_metrics_crowdsec_nginx_bouncer.conf", "crowdsec-nginx-bouncer/v1.0.8")
        if ok == nil then
...
server {
    listen unix:/tmp/crowdsec-metrics-test.sock;
