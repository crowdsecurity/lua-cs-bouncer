...
=== TEST 19: Stream mode with Unix socket LAPI
...
{
        cs = require "crowdsec"
        local ok, err = cs.init("./t/conf_t/19_stream_unix_socket_crowdsec_nginx_bouncer.conf", "crowdsec-nginx-bouncer/v1.0.8")
        if ok == nil then
...
server {
    listen unix:/tmp/crowdsec-stream-test.sock;
